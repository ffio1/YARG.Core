        ...
        private protected static ScanExpected<long> ParseMidi(in FixedArray<byte> file, ref AvailableParts parts, ref DrumsType drumsType)
        {
            ...
            foreach (var track in midiFile)
            {
                ...
                switch (type)
                {
                    ...
                    case MidiTrackType.Elite_Keys_E: if (!parts.EliteKeys[Difficulty.Easy] && Midi_EliteKeys_Preparser.Parse(track)) parts.EliteKeys.ActivateDifficulty(Difficulty.Easy); break;
                    case MidiTrackType.Elite_Keys_M: if (!parts.EliteKeys[Difficulty.Medium] && Midi_EliteKeys_Preparser.Parse(track)) parts.EliteKeys.ActivateDifficulty(Difficulty.Medium); break;
                    case MidiTrackType.Elite_Keys_H: if (!parts.EliteKeys[Difficulty.Hard] && Midi_EliteKeys_Preparser.Parse(track)) parts.EliteKeys.ActivateDifficulty(Difficulty.Hard); break;
                    case MidiTrackType.Elite_Keys_X: if (!parts.EliteKeys[Difficulty.Expert] && Midi_EliteKeys_Preparser.Parse(track)) parts.EliteKeys.ActivateDifficulty(Difficulty.Expert); break;
