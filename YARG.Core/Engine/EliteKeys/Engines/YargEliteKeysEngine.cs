...
namespace YARG.Core.Engine.EliteKeys.Engines
{
    public class YargEliteKeysEngine : EliteKeysEngine
    {
        public YargEliteKeysEngine(InstrumentDifficulty<EliteKeysNote> chart, SyncTrack syncTrack,
            EliteKeysEngineParameters engineParameters, bool isBot) : base(chart, syncTrack, engineParameters, isBot)
        {
        }

        protected override void MutateStateWithInput(GameInput gameInput)
        {
            var action = gameInput.GetAction<EliteKeysAction>();

            if (action is EliteKeysAction.StarPower)
            {
                IsStarPowerInputActive = gameInput.Button;
            }
            else if (action is EliteKeysAction.TouchEffects)
            {
        ...
        protected override void CheckForNoteHit()
        {
            ...
            if (KeyHit != null)
            {
                static EliteKeysNote? CheckForAdjacency(EliteKeysNote fullNote, int key)
                {
                    foreach (var note in fullNote.AllNotes)
                    {
                        if (EliteKeysUtilities.IsAdjacentKey(note.Key, key))
                        {
                            return note;
                        }
                    }
        ...
        protected override bool CanNoteBeHit(EliteKeysNote note)
        {
