...
{
    public abstract class EliteKeysEngine : BaseEngine<EliteKeysNote, EliteKeysEngineParameters,
        EliteKeysStats>
    {
        ...
        public BigInteger KeyMask { get; protected set; }

        public BigInteger PreviousKeyMask { get; protected set; }

        protected double[] KeyPressTimes = new double[(int) EliteKeysAction.C8 + 1];
        ...
        protected EliteKeysNote? FatFingerNote;

        protected EliteKeysEngine(InstrumentDifficulty<EliteKeysNote> chart, SyncTrack syncTrack,
            EliteKeysEngineParameters engineParameters, bool isBot)
            : base(chart, syncTrack, engineParameters, true, isBot)
        {
            ...
            KeyPressTimes = new double[(int) EliteKeysAction.C8 + 1];
        ...
        protected override bool CanSustainHold(EliteKeysNote note)
        {
            return (KeyMask & note.DisjointMask) != 0;
        }

        protected override void HitNote(EliteKeysNote note)
        {
        ...
        protected override void MissNote(EliteKeysNote note)
        {
            if (note.WasHit || note.WasMissed)
            {
                YargLogger.LogFormatTrace("Tried to hit/miss note twice (Key: {0}, Index: {1}, Hit: {2}, Missed: {3})",
        ...
        protected override void AddScore(EliteKeysNote note)
        {
        ...
        protected bool IsKeyInTime(EliteKeysNote note, double frontEnd) => IsKeyInTime(note, note.Key, frontEnd);
    }
}